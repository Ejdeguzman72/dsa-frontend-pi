...
const fetchRestaurantList = async (restaurantTypeId) => {
    try {
        ...
        if (selectedType && selectedType > 0) {
            response = await axiosWithToken.get(`http://192.168.1.36:8080/app/restaurants/all/search/type/${selectedType}`);
        } else {
            response = await axiosWithToken.get('http://192.168.1.36:8080/app/restaurants/all');
        }
...
const renderPagination = () => {
    const totalPages = Math.ceil(restaurants.length / itemsPerPage);  // Based on filtered recipes
